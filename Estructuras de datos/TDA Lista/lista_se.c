...
bool lista_nula(lista_se_t* lista){
	return (lista == NULL);
}
...
lista_se_t* crear_lista(){
	lista_se_t* lista = (lista_se_t*)malloc(sizeof(lista_se_t));
	if(lista_nula(lista)){
		return NULL;
	} else {
		lista->inicio   = NULL;
 ...
 */
nodo_t* buscar_nodo(lista_se_t* lista, size_t posicion){
